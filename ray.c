...
void getRayOrthoPnt(const int mouseX, const int mouseY,
	const int originX, const int originY,
	const int width, const int height,
	const float viewMat[16], const float proMat[16], float pnt[3]) {
	
	float invertMat[16], eyeCoord[4], world[4];
	float pos[4] = { 2.0 * (mouseX - originX) / (width * 1.0) - 1.0, -(2.0 * (mouseY - originY) / (height * 1.0) - 1.0), -1.0, 1.0 };
	//printf("x:%f, y:%f, z:%f\n", pos[0], pos[1], pos[2]);

	inverse4fv(proMat, invertMat);
	multMat4x4fv(invertMat, pos, eyeCoord);
	inverse4fv(viewMat, invertMat);
	multMat4x4fv(invertMat, eyeCoord, world);
